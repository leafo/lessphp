...
class lessc
{
	...
	public $imPrefix = '!';
	public $selfSelector = '&';
	static private $precedence = array('+' => 0, '-' => 0, '*' => 1, '/' => 1, '%' => 1,);
	...
	function chunk()
	{
		...
		if ($this->tags($tags, true, '>') && ($this->argumentValues($argv) || true) && $this->end()) {
			...
			if ($this->level == 1) {
				return $this->compileBlock(null, $env);
			} else {
				return true;
			}
		} else {
	...
	{
		return (count($items) == 1) ? $items[0] : array('list', $delim, $items);
	}
